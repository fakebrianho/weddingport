...
export default class InfiniteGrid {
	...
	openLightbox(imgSrc) {
		...
		// Calculate proper scaling to fit image within viewport
		lightboxImg.onload = () => {
			...
			if (imgAspectRatio > viewportAspectRatio) {
				...
				maxWidth = '90vw'
				maxHeight = `${90 / imgAspectRatio}vw`
			} else {
				// Image is taller than viewport - constrain by height
				maxHeight = '90vh'
				maxWidth = `${90 * imgAspectRatio}vh`
			}
			
			lightboxImg.style.maxWidth = maxWidth
			lightboxImg.style.maxHeight = maxHeight
		}
			...
			repsX.forEach((offsetX) => {
				repsY.forEach((offsetY) => {
					...
					split.lines.forEach((line, i) => {
						...
						line.parentElement.style.transitionDelay = `${
							i * 0.15
						}s`
					})
	...
	onMouseUp(e) {
		...
		// If it's a short click with minimal movement, treat as click
		if (clickDuration < 200 && clickDistance < 10) {
			const clickedElement = e.target.closest('.item-image')
			if (clickedElement) {
