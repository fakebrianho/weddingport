...
export default class Index {
	...
	initScrollBehavior() {
		...
		// Ensure the scrollable container can receive mouse wheel events
	...
	initDynamicScroll() {
		...
		const checkScrollNeeded = () => {
			...
			const containerHeight = contactContent.clientHeight - 200 // Account for tabs and padding

			if (contentHeight > containerHeight) {
				tabContent.style.maxHeight = `${containerHeight}px`
				tabContent.style.overflowY = 'auto'
			} else {
				tabContent.style.maxHeight = 'none'
				tabContent.style.overflowY = 'visible'
			}
		}
		...
		tabButtons.forEach((button) => {
			button.addEventListener('click', () => {
				setTimeout(() => {
					checkScrollNeeded()
					// Add overflow for booking tab, remove for contact tab
					const targetTab = button.getAttribute('data-tab')
					if (targetTab === 'book') {
						tabContent.style.overflowY = 'auto'
					} else {
	...
	showToast(message, type = 'success') {
		...
		setTimeout(() => {
			...
			// If it's a success message, close the contact page after toast fades out
			if (type === 'success') {
				setTimeout(() => {
					const contactPage = document.getElementById('contact-page')
					const transitionDiv1 =
						document.querySelector('.transition-div-1')
					const transitionDiv2 =
						document.querySelector('.transition-div-2')
				...
				}, 400) // Wait for toast fade out animation to complete
			}
		}, 1500)
	}
}
